...
import { ButtonBehavior } from './button.ts';
...
const buttonBehavior = new ButtonBehavior(root);
await buttonBehavior.init();
...
/*
const circleCutoutDist = (position: d.v2f) => {
  'use gpu';
  const groundRoundness = GroundParams.groundRoundness;
  const groundRadius = GroundParams.groundRadius;

  return sdf.sdCircle(position, groundRadius + groundRoundness);
};
*/
...
const circleCutoutDist = (position: d.v2f) => {
  ...
  return std.length(position) - (groundRadius + groundRoundness);
};
...
const getJellyDist = (position: d.v3f) => {
  'use gpu';
  const state = buttonBehavior.stateUniform.$;

  // Base center position (where the button is "glued")
  const baseCenter = d.vec3f(0, 0.3, 0.0);

  // Calculate local position relative to base
  const localToBase = position.sub(baseCenter);

  // Normalized height factor (0 at bottom, 1 at top)
  // Adjust JELLY_HALFSIZE.y * 2 to match your button's actual height
  const heightFactor = std.saturate(
    (localToBase.y + JELLY_HALFSIZE.y) / (JELLY_HALFSIZE.y * 2)
  );

  // Apply displacement based on height - bottom stays, top moves
  const displacement = d.vec3f(
    (state.posX * heightFactor) / 15, // X displacement increases with height
    (state.posY * heightFactor) / -18, // Y handled separately by squashing
    (state.posZ * heightFactor) / 15 // Z displacement increases with height
  );

  // Apply the displacement
  const displacedPos = localToBase.sub(displacement);

  // Apply squashing (this affects the whole volume)
  const jellyInvScale = d.vec3f(
    1.0, /// (1.0 - state.squashX * 0.5 + 0.001),
    1.0,
    1.0 / (1.0 - state.squashZ * 0.5 + 0.001)
  );

  const scaledPos = displacedPos.mul(jellyInvScale);
...
const getNormal = (position: d.v3f) => {
  'use gpu';
  if (std.abs(position.z) > 0.5 || std.abs(position.x) > 0.6) {
...
const getFakeShadow = (position: d.v3f, lightDir: d.v3f): d.v3f => {
  ...
  if (position.y < -GroundParams.groundThickness) {
    ...
    const inset = 0.02;
    const cutout = circleCutoutDist(position.xz) + inset;
...
const renderBackground = (
  ...
  // Calculate fake bounce lighting from button center
  const jellyColor = jellyColorUniform.$;
  const sqDist = sqLength(hitPosition.sub(d.vec3f(0, 0, 0)));
  ...
  const backgroundColor = applyAO(
    std
      .select(LIGHT_GROUND_ALBEDO, DARK_GROUND_ALBEDO, darkModeUniform.$ === 1)
      .mul(litColor),
    hitPosition,
    newNormal
  )
...
function render(timestamp: number) {
  ...
  buttonBehavior.update(deltaTime);
...
canvas.addEventListener('touchstart', (event) => {
  buttonBehavior.pressed = true;
  event.preventDefault();
});

canvas.addEventListener('touchend', (_event) => {
  buttonBehavior.pressed = false;
});
...
async function autoSetQuaility() {
  ...
  const measurePipeline = rayMarchPipeline.withPerformanceCallback(
    (start, end) => {
